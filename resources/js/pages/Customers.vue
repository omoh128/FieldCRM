    ...
    <main class="main">
      ...
      <header class="topbar">
        ...
        <div class="topbar-right">
          <button class="btn btn-ghost" @click="exportCustomers">
      ...
      <div class="content">
        ...
        <div class="stats-row">
          ...
          <div class="stat-card stat-green">
            ...
            <div class="stat-sub delta-up">Engaged accounts</div>
          </div>
          <div class="stat-card stat-blue">
            <div class="stat-label">Total Lifetime Rev.</div>
        ...
        <div class="toolbar">
          <div class="search-wrap">
            ...
            <input v-model="searchQuery" class="search-input" placeholder="Search customers..." />
          </div>
          ...
          <span class="count-label">{{ filteredCustomers.length }} customer{{ filteredCustomers.length !== 1 ? 's' : '' }}</span>
        </div>
              ...
              >
                <td>
                  <div class="entity-cell">
                    <div class="e-avatar" :style="{ background: customerColor(customer) + '22', color: customerColor(customer) }">
                      {{ initials(customer) }}
                    </div>
                    <div>
                      <div class="e-name">{{ customer.firstName }} {{ customer.lastName }}</div>
                      <div class="e-sub">{{ customer.company || '—' }}</div>
                    </div>
                  </div>
                </td>
                <td><span :class="['badge', `badge-${customer.status}`]">{{ capitalize(customer.status) }}</span></td>
                <td class="text-muted2">{{ capitalize(customer.type) }}</td>
                <td class="text-muted2">{{ customer.phone || '—' }}</td>
                <td class="text-muted text-sm">{{ shortAddress(customer.address) }}</td>
                <td class="text-center text-muted2">{{ customer.jobs }}</td>
                <td class="value-cell">${{ customer.value.toLocaleString() }}</td>
                <td class="text-muted">{{ formatDate(customer.lastContact) }}</td>
                <td>
                  <div class="row-actions" @click.stop>
                    <button class="action-btn" title="Edit" @click="openEdit(customer)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                      </svg>
                    </button>
                    <button class="action-btn del" title="Delete" @click="askDelete(customer.id)">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div v-if="showFormModal" class="modal-overlay" @click.self="closeFormModal">
        <div class="modal">
          <div class="modal-header">
            <span class="modal-title">{{ editingId ? 'Edit Customer' : 'Add Customer' }}</span>
          ...
          <div class="modal-body">
            <div class="form-grid">
              <div class="form-group">
                ...
                <input v-model="form.firstName" class="form-input" placeholder="Jane" />
              </div>
              <div class="form-group">
                <label class="form-label">Last Name *</label>
                <input v-model="form.lastName" class="form-input" placeholder="Smith" />
              </div>
              <div class="form-group full">
                <label class="form-label">Company</label>
                <input v-model="form.company" class="form-input" placeholder="Smith Properties" />
              </div>
              <div class="form-group">
                <label class="form-label">Email</label>
                <input v-model="form.email" class="form-input" placeholder="jane@example.com" />
              </div>
              <div class="form-group">
              ...
              <div class="form-group full">
                ...
                <input v-model="form.address" class="form-input" placeholder="123 Main St, City, State" />
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input v-model.number="form.jobs" type="number" class="form-input" placeholder="0" />
              </div>
              <div class="form-group full">
                <label class="form-label">Tags (comma separated)</label>
                <input v-model="form.tagsRaw" class="form-input" placeholder="vip, repeat, referral" />
              </div>
              <div class="form-group full">
                <label class="form-label">Notes</label>
                <textarea v-model="form.notes" class="form-textarea" placeholder="Customer notes..."></textarea>
              </div>
            </div>
          </div>
          <div class="modal-footer">
            <button class="btn btn-ghost" @click="closeFormModal">Cancel</button>
            <button class="btn btn-primary" @click="saveCustomer">Save Customer</button>
          </div>
        </div>
      </div>
    </transition>
          ...
          <div class="modal-body">
            <div class="detail-section">
              <div class="detail-hero">
                <div class="e-avatar detail-avatar" :style="{ background: customerColor(selectedCustomer) + '22', color: customerColor(selectedCustomer) }">
                  {{ initials(selectedCustomer) }}
                </div>
                <div>
                  <div class="detail-name">{{ selectedCustomer.firstName }} {{ selectedCustomer.lastName }}</div>
                  <div class="detail-company">{{ selectedCustomer.company || 'No company' }}</div>
                  <div class="detail-tags">
                    <span v-for="tag in selectedCustomer.tags" :key="tag" class="tag">{{ tag }}</span>
                  </div>
                </div>
                <span :class="['badge', `badge-${selectedCustomer.status}`]" style="margin-left:auto">
                  {{ capitalize(selectedCustomer.status) }}
                </span>
              </div>
              <div class="detail-grid">
                <div class="detail-item"><div class="detail-label">Email</div><div class="detail-value">{{ selectedCustomer.email || '—' }}</div></div>
                <div class="detail-item"><div class="detail-label">Phone</div><div class="detail-value">{{ selectedCustomer.phone || '—' }}</div></div>
                <div class="detail-item"><div class="detail-label">Type</div><div class="detail-value">{{ capitalize(selectedCustomer.type) }}</div></div>
                <div class="detail-item"><div class="detail-label">Total Jobs</div><div class="detail-value">{{ selectedCustomer.jobs }}</div></div>
                <div class="detail-item"><div class="detail-label">Lifetime Value</div><div class="detail-value value-cell">${{ selectedCustomer.value.toLocaleString() }}</div></div>
                <div class="detail-item"><div class="detail-label">Last Contact</div><div class="detail-value">{{ formatDate(selectedCustomer.lastContact) }}</div></div>
                <div class="detail-item detail-full"><div class="detail-label">Address</div><div class="detail-value">{{ selectedCustomer.address || '—' }}</div></div>
              </div>
            </div>
            <div v-if="selectedCustomer.notes" class="detail-section">
              <h4>Notes</h4>
              <p class="detail-notes">{{ selectedCustomer.notes }}</p>
            </div>
          </div>
          <div class="modal-footer">
            <button class="btn btn-ghost" @click="showDetailModal = false">Close</button>
            <button class="btn btn-primary" @click="openEditFromDetail">Edit Customer</button>
          </div>
        </div>
      </div>
    </transition>
    ...
    <transition name="fade">
      <div v-if="showConfirmModal" class="confirm-overlay" @click.self="showConfirmModal = false">
        <div class="confirm-box">
          ...
          <p>All associated data will be permanently removed. This cannot be undone.</p>
...
const normalise = (c) => ({
  ...
  jobs:        c.jobs_count || c.jobs || 0,
  value:       parseFloat(c.value) || 0,
  tags:        Array.isArray(c.tags) ? c.tags : [],
  lastContact: (c.updated_at || c.created_at || '').split('T')[0],
})

const initials      = (c) => ((c.firstName||'')[0]||'') + ((c.lastName||'')[0]||'')
const customerColor = (c) => COLORS[(c.id - 1) % COLORS.length]
const capitalize    = (s) => s ? s.charAt(0).toUpperCase() + s.slice(1) : ''
const formatDate    = (d) => d
  ? new Date(d + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
  : '—'
const shortAddress  = (addr) => addr ? addr.split(',').slice(-2).join(',').trim() : '—'
...
const fetchCustomers = async () => {
  ...
  try {
    ...
    customers.value = (data.data ?? data).map(normalise)
  } catch (err) {
    console.error(err)
    customers.value = []
  } finally {
...
const totalPages     = computed(() => Math.max(1, Math.ceil(filteredCustomers.value.length / PAGE_SIZE)))
...
const stats = computed(() => {
  ...
  const revenue = customers.value.reduce((a, c) => a + c.value, 0)
  return { total, active: total, revenue, avg: total ? Math.round(revenue / total) : 0 }
})
...
const openDetail         = (c)  => { selectedCustomer.value = c; showDetailModal.value = true }
const openAddModal       = ()   => {} // customers come from converting leads
...
const confirmDelete = async () => {
  try {
    ...
    customers.value = customers.value.filter(c => c.id !== deletingId.value)
...
const exportCustomers = () => {
  const rows = filteredCustomers.value.map(c => ({
    Name: `${c.firstName} ${c.lastName}`, Company: c.company || '',
    Email: c.email || '', Phone: c.phone || '', Value: c.value, Jobs: c.jobs,
  }))
